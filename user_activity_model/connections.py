...
from constants import AWS_ACCESS_KEY_ID_ENV_KEY,AWS_REGION_NAME,AWS_SECRET_ACCESS_KEY_ENV_KEY,S3_TRAINING_BUCKET_NAME,S3_FEAST_FEATURE_STORE,PRODUCTION_MODEL_FILE_PATH,FEATURE_STORE_FILE_PATH
import sys
#import certifi
...
class AwsStorage:
    def __init__(self):
        try:
            ...
            self.FEAST_BUCKET_NAME = S3_FEAST_FEATURE_STORE
...
class StorageConnection:
    ...
    def __init__(self):
        try:
            ...
            self.feature_repo = self.s3.Bucket(self.config.FEAST_BUCKET_NAME)
        except Exception as e:
            raise PredictionException(e,sys)
    ...
    def get_feature_store_s3(self):
        ...
        """
        try:
            s3_folder = "feature_repo"
            local_dir = FEATURE_STORE_FILE_PATH
            bucket = self.bucket
            for obj in bucket.objects.filter(Prefix=s3_folder):
                target = obj.key if local_dir is None \
                    else os.path.join(local_dir, os.path.relpath(obj.key, s3_folder))
...
if __name__ == "__main__":
    ...
    connection.get_feature_store_s3()
